function ada_trainingAdaBoost(strongName, trainFeatureMtxCell);

% load('ada_trainFeatureMtxCell');

answer = inputdlg('AdaBoost Running Cycles?','Adaboost Cycle')
...
for firstSymbol = 1:symbolNum-1;
    ...
    for secondSymbol = firstSymbol + 1: symbolNum
        ...
        for cycle = 1:T*J;
            ...
            alphaMtx(featureInd,secondSymbol,firstSymbol) = real(alpha);
        end
...
close(bar);
name1 = [strongName , '_strongClassifier'];
name2 = [strongName , '_classifierMtx'];
name3 = [strongName, '_alphaMtx'];
save(name1 , 'strongClassifier');
save(name2 , 'classifierMtx');
save(name3 , 'alphaMtx');
